...
The private-alpha store defaults to `var/private-alpha-store.json`. For real hosting, set `AURA_WEB_DB_PATH` to a persistent disk path or replace `AlphaStore` with Postgres before scaling beyond a small alpha.
