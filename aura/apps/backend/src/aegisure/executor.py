...
def _record_step_history(run_id: str, step, status: str, **extra):
    ...
    if status in {'needs_confirmation', 'blocked', 'success', 'terminal_failure', 'needs_user'}:
        payload = {
            ...
            'identity_name': identity.get('name', 'Personal Aegisure'),
        ...
        record_audit_event({
            ...
            'message': f"Aegisure acted under {identity.get('name') or 'Personal Aegisure'}: {step.name} -> {status}.",
            'payload': payload,
        })
...
def execute_steps(run_id: str, steps, event_cb, wait_poll_ms: int = 100, start_index: int = 0):
    ...
    for idx, step in enumerate(steps[start_index:], start=start_index):
        ...
        if guard == 'blocked':
            ...
            _emit_step(event_cb, run_id, step, 'failed', message=f"Aegisure Guardian blocked this action: {event['message']}", url=last_obs.get('url', ''), guardian_reason=event['message'])
            return out
        if guard == 'confirm' and step.action_type not in (ASSIST_APPROVAL, ASSIST_PASTE) and not is_step_approved(run_id, step.id):
            ...
            guardian_event = decision_event(
                ...
                action_required='Approve, reject, or edit this action before Aegisure continues.',
            )
            record_guardian_event(guardian_event)
            append_run_history(run_id, 'guardian_events', guardian_event, limit=40)
            ...
            payload = {
                ...
                'step_index': idx,
                'approval_id': approval['approval_id'],
                'message': 'Confirmation required before Aegisure can continue.',
                'action_type': step.action_type,
                'risk_reason': approval['risk_reason'],
                'risk_level': risk_decision.get('risk'),
