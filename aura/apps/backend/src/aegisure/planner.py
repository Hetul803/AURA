...
def _note_plan(text: str, context: dict | None = None) -> dict:
    ...
    return _build_plan(
        goal='Create a note in the approved Aegisure workspace',
        signature='file:note',
        steps=[
            Step(id='s1', name='Write workspace note', action_type='FS_WRITE_TEXT', tool='filesystem', args={'path': path, 'text': f'# Aegisure Note\n\n{body}\n'}, expected_outcome={'written_gte': len(body)}, safety_level='CONFIRM'),
...
def _daily_operator_plan(text: str, context: dict | None = None) -> dict:
    return _build_plan(
        ...
        steps=[
            ...
            Step(id='s2', name='Summarize current session readiness', action_type='NOOP', tool='control', args={'message': 'Aegisure checked identity, Guardian, memory, context, and pending approvals for this work session.'}, expected_outcome={'ok': True}),
        ],
