...
def _run_memory_request(run_id: str, text: str, planning_context: dict, event_cb) -> dict | None:
    ...
    record_audit_event({
        ...
        'message': f"Aegisure acted under {identity.get('name') or identity.get('identity_id')} to evaluate memory storage.",
...
def run_command(text: str, event_cb=lambda e: None, choices: dict | None = None, use_macro: bool = False, run_id: str | None = None, context: dict | None = None, proactive: dict | None = None):
    ...
    if plan.get('signature') == 'user_ai:web':
        ...
        if privacy_check.get('requires_approval') or privacy_check.get('redacted'):
            event = record_human_guardian_event(
                ...
                explanation=f"Destination: {privacy_check.get('destination') or 'external AI tool'}. {privacy_check.get('approval_reason') or 'Aegisure prepared a privacy-checked prompt.'}",
                action_required='Review the prompt before Aegisure copies or pastes it into another AI tool.',
                run_id=run_id,
    ...
    record_audit_event({
        ...
        'action_type': 'COMMAND',
        'risk_level': 'low',
        'message': f"Aegisure acted under {identity_context['identity_name']}.",
        'payload': audit_payload,
    })
