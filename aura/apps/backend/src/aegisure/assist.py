...
def gather_context(*, request_text: str, captured_context: dict, research_mode: str) -> dict:
    ...
    step = SimpleNamespace(action_type='WEB_READ', args={'target': 'search', 'query': query, 'use_fixture': os.getenv('AEGISURE_FORCE_FIXTURES') == '1'})
...
def capture_structured_context(raw_context: dict | None = None) -> dict:
    ...
    if raw_context is None and os.getenv('AEGISURE_FORCE_FIXTURES') == '1' and using_original_capture:
