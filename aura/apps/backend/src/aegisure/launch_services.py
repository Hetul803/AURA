...
def _license_server() -> str:
    return (os.getenv('AEGISURE_LICENSE_SERVER_URL') or '').rstrip('/')


def _update_feed_url() -> str:
    explicit = (os.getenv('AEGISURE_UPDATE_FEED_URL') or '').strip()
    if explicit:
        return explicit
...
def launch_status() -> dict[str, Any]:
    ...
    crash_url = (os.getenv('AEGISURE_CRASH_REPORT_URL') or '').strip() or (f'{_license_server()}/api/crash-reports' if _license_server() else '')
    return {
        'ok': True,
        'checked_at': _now(),
        'brand': {
...
def check_for_updates(current_version: str = '1.0.0', platform_name: str | None = None, arch: str | None = None) -> dict[str, Any]:
    ...
    if not feed:
        ...
        return {
            ...
            'message': 'Set AEGISURE_UPDATE_FEED_URL or AEGISURE_LICENSE_SERVER_URL to enable update checks. Manual DMG updates still work.',
...
def report_crash(payload: dict[str, Any]) -> dict[str, Any]:
    crash_url = (os.getenv('AEGISURE_CRASH_REPORT_URL') or '').strip() or (f'{_license_server()}/api/crash-reports' if _license_server() else '')
    report = {
        'product': get_brand().get('product_name') or 'Aegisure',
        'reported_at': _now(),
        'system': {
            'platform': platform.system(),
            'release': platform.release(),
    ...
    if not crash_url:
        return {
            ...
            'message': 'Crash report was redacted locally. Set AEGISURE_CRASH_REPORT_URL or AEGISURE_LICENSE_SERVER_URL to upload reports.',
            'report': report,
        }
    try:
...
def launch_env_template() -> str:
    return json.dumps({
        'website': {
            ...
            'AEGISURE_VENDOR_PRIVATE_KEY': '-----BEGIN PRIVATE KEY-----...',
            'AEGISURE_VENDOR_PUBLIC_KEY': '-----BEGIN PUBLIC KEY-----...',
            'AEGISURE_ADMIN_TOKEN': 'generate-a-long-random-token',
            'AEGISURE_DOWNLOAD_MAC_URL': 'https://your-domain.com/downloads/Aegisure-1.0.0-mac-arm64.dmg',
        },
        'desktop_backend': {
            'AEGISURE_LICENSE_PUBLIC_KEY': '-----BEGIN PUBLIC KEY-----...',
