...
def _clipboard_event(run_id: str, step, result: dict, run_context: dict) -> list[dict]:
    ...
    if action in {'OS_READ_CLIPBOARD', 'OS_COPY_SELECTION', 'ASSIST_CAPTURE_CONTEXT'}:
        ...
        summary = 'Aegisure read from the clipboard to capture context.' if action == 'ASSIST_CAPTURE_CONTEXT' else 'Aegisure accessed clipboard content.'
        explanation = 'This happened during context capture or selection copy so Aegisure could work with the active text.'
        action_required = 'No action needed.'
        risk = LOW
        if source == 'clipboard_fallback' or size >= 800:
            risk = MEDIUM
            summary = 'Aegisure used the clipboard fallback to capture context.'
            explanation = 'No reliable text selection was available, so Aegisure relied on clipboard contents from the active app.'
        ...
        if _repetition_count(run_id, event_type) >= 1:
            risk = MEDIUM
            explanation = 'Aegisure accessed clipboard content multiple times during this run.'
        context = {
            'target': source,
            'size': size,
    ...
    risk = LOW
    summary = 'Aegisure prepared clipboard content for paste-back.' if action == 'ASSIST_PASTE_BACK' else 'Aegisure wrote content to the clipboard.'
    explanation = 'This is part of Aegisure placing generated or requested text into the active app.'
    action_required = 'Review the target and approve paste-back before Aegisure changes another app.'
    if size >= 600 or action in {'OS_PASTE', 'ASSIST_PASTE_BACK'}:
        risk = MEDIUM
        summary = 'Aegisure pasted or staged a larger block of content.'
        explanation = 'Aegisure placed a sizable result onto the clipboard and attempted paste-back into the active target.'
    if size >= 1800:
        risk = HIGH
        summary = 'Aegisure pasted a large block of content into another app.'
        explanation = 'Large paste-back actions can move substantial text into the active app, so Aegisure marks them as higher risk.'
    if _repetition_count(run_id, 'clipboard_write', str(target)) >= 1:
        risk = HIGH if risk in {MEDIUM, HIGH} else MEDIUM
        explanation = 'Aegisure interacted with the clipboard repeatedly for this target during the same run.'
    context = {
        'target': str(target),
        'size': size,
...
def _file_event(run_id: str, step, result: dict) -> list[dict]:
    ...
    if operation == 'write' and size >= 2000:
        risk = MEDIUM
    summary = f"Aegisure {operation} a local file."
    explanation = 'This event is limited to filesystem actions Aegisure performed inside its own workflow.'
...
def _browser_events(run_id: str, step, result: dict, run_context: dict) -> list[dict]:
    ...
    if action == 'ASSIST_RESEARCH_CONTEXT':
        ...
        risk = HIGH
        summary = 'Aegisure performed an external research query.'
        ...
        if _repetition_count(run_id, 'network_action') >= 1:
            explanation = 'Aegisure used external research multiple times during this run.'
    ...
    risk = LOW
    summary = 'Aegisure accessed a browser context.'
    explanation = 'This reflects Aegisure visiting or reading from a page that it can observe in its workflow.'
    ...
    if action in {'WEB_READ', 'WEB_UPLOAD'} and (step.args.get('target') == 'search' or _domain(url) not in {None, '', 'localhost', '127.0.0.1'}):
        ...
        summary = 'Aegisure reached an external web destination.'
    ...
    if action == 'WEB_UPLOAD':
        events.append(_base_event(
            ...
            event_type='network_action',
            risk=HIGH,
            summary='Aegisure prepared a browser upload action.',
            explanation='Uploading through a browser can move a local file into an external destination, so Aegisure flags it as high risk.',
