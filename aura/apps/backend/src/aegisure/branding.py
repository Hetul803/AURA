...
DEFAULT_BRAND = {
    'product_name': 'Aegisure',
    'assistant_default_name': 'Aegisure',
    'company_name': 'Aegisure Labs',
...
def get_brand() -> dict[str, Any]:
    ...
    env = {
        key: value for key, value in {
            'product_name': os.getenv('AEGISURE_PRODUCT_NAME'),
            'assistant_default_name': os.getenv('AEGISURE_ASSISTANT_NAME'),
            'company_name': os.getenv('AEGISURE_COMPANY_NAME'),
            'support_url': os.getenv('AEGISURE_SUPPORT_URL'),
            'download_url': os.getenv('AEGISURE_DOWNLOAD_URL'),
        }.items() if value
    }
    brand = {**DEFAULT_BRAND, **_file_brand(), **stored, **env}
    brand['license_public_key_configured'] = bool(os.getenv('AEGISURE_LICENSE_PUBLIC_KEY') or os.getenv('AEGISURE_VENDOR_PUBLIC_KEY'))
    return brand
