...
def _current_folder() -> str:
    configured = os.getenv('AEGISURE_CURRENT_FOLDER') or os.getenv('AEGISURE_WORKSPACE')
    if configured:
...
def capture_current_context(*, source: str = 'desktop', persist: bool = True) -> dict:
    if os.getenv('AEGISURE_FORCE_FIXTURES') == '1':
        raw = {
            'ok': True,
            'active_app': 'Fixture Browser',
            'window_title': 'Aegisure fixture context',
            'browser_url': 'https://github.com/Hetul803/Aegisure',
            'browser_title': 'Hetul803/Aegisure',
            'selected_text': 'Fixture selected text for Aegisure tests.',
            'clipboard_text': '',
            'input_text': 'Fixture selected text for Aegisure tests.',
            ...
            'target_fingerprint': {'app_name': 'Fixture Browser', 'browser_domain': 'github.com', 'browser_url': 'https://github.com/Hetul803/Aegisure'},
            'paste_target': {'app_name': 'Fixture Browser', 'browser_domain': 'github.com', 'browser_url': 'https://github.com/Hetul803/Aegisure'},
        }
    else:
        raw = capture_context()
