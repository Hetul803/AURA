...
def _recommendation_reason(hardware: dict[str, Any], model: dict[str, Any]) -> str:
    ...
    if ram is None:
        return f'{model["label"]} is the conservative default because Aegisure could not read total RAM on this {chip} machine.'
...
def _setup_steps(installed: bool, running: bool, recommendation: dict[str, Any]) -> list[str]:
    ...
    if installed and not running:
        steps.append('Start Ollama, then refresh Aegisure local model status.')
...
def _summary(installed: bool, running: bool, names: list[str], recommendation: dict[str, Any]) -> str:
    if not installed:
        return 'Ollama is not installed. Aegisure can start with SimpleLLM, but real local drafting needs Ollama.'
    if not running:
        return 'Ollama is installed but not running.'
    if recommendation.get('installed'):
        return f"Aegisure found local models and recommends {recommendation['model']} for this Mac."
    return f"Ollama is running with {len(names)} model(s). Aegisure recommends pulling {recommendation['recommended_pull']}."
...
def start_ollama_server(wait_seconds: float = 2.0) -> dict[str, Any]:
    if shutil.which('ollama') is None:
        return {
            ...
            'install_url': 'https://ollama.com/download',
            'install_command': 'brew install --cask ollama',
            'message': 'Ollama is not installed, so Aegisure cannot start the local model runtime.',
        }
    if ollama_available():
    ...
    return {
        ...
        'message': 'Aegisure tried to start Ollama, but it was not reachable yet. Start the Ollama app or run `ollama serve`, then retry.',
    }
...
def pull_model(model: str, *, approved: bool = False, select_after_pull: bool = True, timeout: int = 1800) -> dict[str, Any]:
    ...
    if shutil.which('ollama') is None:
        return {
            ...
            'install_url': 'https://ollama.com/download',
            'install_command': 'brew install --cask ollama',
            'repair': 'Install Ollama, open it once, then retry the download in Aegisure.',
        }
    if not ollama_available():
        ...
        if not started.get('ok'):
            return {
                ...
                'repair': 'Start the Ollama app or run `ollama serve`, then retry the download in Aegisure.',
            }
