...
app = typer.Typer(help="AURA trust, memory, and control layer for AI coding agents.")
...
def init(
    ...
    overwrite: bool = typer.Option(False, "--overwrite", help="Overwrite an existing AURA.md."),
    print_only: bool = typer.Option(False, "--print", help="Print the generated Constitution instead of writing AURA.md."),
) -> None:
    """Scan a repository and create its AURA.md Constitution."""
...
def scan(
    ...
    typer.echo(f"AURA risk report: {report.verdict.upper()} ({report.score}/100)")
...
def export(
    ...
    """Write cross-agent memory files from the canonical AURA Constitution."""
