...
from .assist import analyze_approved_edit, apply_feedback_preferences, draft_from_state
from .demo import build_demo_metadata
...
from .timing import initial_hero_timing, mark_hero_timing, set_hero_phase
...
def run_command(text: str, event_cb=lambda e: None, choices: dict | None = None, use_macro: bool = False, run_id: str | None = None, proactive: dict | None = None):
    ...
    if plan['signature'] == 'unsupported:command':
        unsupported_message = ((plan.get('context') or {}).get('unsupported_reason') or 'Unsupported command.')
        supported_examples = ((plan.get('context') or {}).get('supported_examples') or [])
        ...
        event_cb({'type': 'unsupported_command', 'run_id': run_id, 'status': 'unsupported', 'message': unsupported_message})
        return {
            'ok': False,
            'run_id': run_id,
            'status': 'unsupported',
            ...
            'run_state': get_run_context(run_id),
        }
    ...
    assist_run = plan.get('signature') == 'assist:writing'
    demo = build_demo_metadata(proactive)
    set_run_context(run_id, {
        'run_id': run_id,
        'text': text,
        'choices': choices or {},
        ...
        'pasteback_state': {'status': 'not_started'},
        'hero_timing': initial_hero_timing(proactive=proactive, assist_run=assist_run),
        'hero_phase': 'capturing' if assist_run else 'running',
        'demo': demo,
        'assist': {
    ...
    if assist_run:
        set_hero_phase(run_id, 'capturing', label='Capturing context', detail='Gathering the current app context.')
    ...
    if status == 'awaiting_approval':
        set_hero_phase(run_id, 'awaiting_approval', label='Ready to review', detail='Draft ready for approval and paste-back.')
    elif status == 'needs_user':
        set_hero_phase(run_id, 'needs_attention', label='Needs attention', detail='AURA needs a manual step before it can continue.')
    else:
        mark_hero_timing(run_id, 'run_completed_at', overwrite=True)
        set_hero_phase(run_id, 'completed' if status == 'done' else 'needs_attention', label='Done' if status == 'done' else 'Needs attention', detail='AURA finished this request.' if status == 'done' else 'AURA could not finish this request automatically.')
...
def resume_run(run_id: str, event_cb=lambda e: None):
    ...
    if (ctx.get('plan') or {}).get('signature') == 'assist:writing' and (ctx.get('approval_state') or {}).get('status') == 'approved':
        set_hero_phase(run_id, 'pasting', label='Pasting', detail='Restoring the target and pasting the approved draft.')
    ...
    if status == 'done':
        mark_hero_timing(run_id, 'run_completed_at', overwrite=True)
        set_hero_phase(run_id, 'completed', label='Done', detail='AURA finished this request.')
    elif status == 'needs_user':
        set_hero_phase(run_id, 'needs_attention', label='Needs attention', detail='AURA needs help before it can continue.')
    learning = record_run_learning(run_id, get_run_context(run_id) or {})
    update_run_context(run_id, {'learning': learning})
    return {'ok': status == 'done', 'run_id': run_id, 'steps': result, 'plan': ctx.get('plan'), 'run_state': get_run_context(run_id)}
...
def approve_assist_run(run_id: str, approved_text: str | None = None, event_cb=lambda e: None):
    ...
    edit_signals = analyze_approved_edit(generated_text, final_text)
    if final_text != generated_text:
        ...
        if edit_signals.get('tone_preference') == 'direct':
            apply_feedback_preferences('more direct', task_kind)
        elif edit_signals.get('tone_preference') == 'polished':
            apply_feedback_preferences('more polished', task_kind)
        if edit_signals.get('warmth_preference') == 'warm':
            apply_feedback_preferences('warmer', task_kind)
        if edit_signals.get('structure_preference') == 'summary_first':
            apply_feedback_preferences('summary first', task_kind)
        elif edit_signals.get('structure_preference') == 'answer_first':
            apply_feedback_preferences('answer first', task_kind)
    ...
    mark_hero_timing(run_id, 'approval_received_at', overwrite=True)
    ...
    update_run_context(run_id, {'assist': {**(ctx.get('assist') or {}), 'edit_signals': edit_signals, 'final_outcome': 'approved_pending_paste'}})
...
def retry_assist_run(run_id: str, feedback: str | None = None, event_cb=lambda e: None):
    ...
    try:
        ...
        set_hero_phase(run_id, 'needs_attention', label='Needs attention', detail='Drafting failed because the assist model was unavailable.')
    ...
    set_hero_phase(run_id, 'awaiting_approval', label='Ready to review', detail='Updated draft ready for approval.')
...
def reject_assist_run(run_id: str, reason: str | None = None, event_cb=lambda e: None):
    ...
    update_run_context(run_id, {'approval_state': approval, 'status': 'rejected', 'terminal_outcome': 'rejected', 'pasteback_state': {'status': 'skipped', 'paste_attempted': False, 'paste_blocked_reason': 'draft_rejected'}, 'assist': {**(ctx.get('assist') or {}), 'learning_signals': {'feedback_preferences': learned}, 'final_outcome': 'rejected'}})
    mark_hero_timing(run_id, 'run_completed_at', overwrite=True)
    set_hero_phase(run_id, 'needs_attention', label='Draft rejected', detail='Paste-back was skipped because the draft was rejected.')
