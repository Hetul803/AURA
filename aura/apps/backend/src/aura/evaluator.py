...
def _expected_outcome_met(step, result: dict, observation_map: dict) -> bool:
    ...
    for key, exp in expected.items():
        ...
        if key == 'written_gte' and int(result.get('written', 0)) < int(exp):
            return False
        if key == 'pasted_gte':
            pasted = int(result.get('pasted', 0))
            copied = int(result.get('copied', 0) or result.get('copy_ready', 0))
            if pasted < int(exp) and not (observation_map.get('demo_copy_fallback') and copied >= int(exp)):
                return False
