...
def configured_public_key() -> str | None:
    raw = os.getenv('AURA_LICENSE_PUBLIC_KEY') or os.getenv('AURA_VENDOR_PUBLIC_KEY')
    if not raw:
        return None
...
def license_status() -> dict[str, Any]:
    ...
    if not row:
        return {
            ...
            'message': 'No paid license is active. AURA runs in local private-alpha mode.',
        }
    item = dict(row)
...
def verify_license_token(token: str) -> dict[str, Any]:
    ...
    if not public_pem:
        return {
            ...
            'message': 'Set AURA_LICENSE_PUBLIC_KEY before activating paid licenses. The app stays in local_free mode.',
        }
