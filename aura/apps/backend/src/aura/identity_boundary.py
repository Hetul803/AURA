...
def ensure_default_identities() -> None:
    ...
    upsert_boundary_policy(
        ...
        reason='Personal memory should not automatically enter work AURA context.',
    )
    upsert_boundary_policy(
        source_identity='work',
        target_identity='personal',
        ...
        action='remember',
        decision='require_approval',
        reason='Work memory should not automatically enter personal AURA context.',
    )
    upsert_boundary_policy(
...
def get_active_identity() -> dict[str, Any]:
    ...
    return identity or {'identity_id': 'personal', 'name': 'Personal AURA', 'memory_scope': 'personal', 'metadata': _default_metadata(memory_scope='personal')}
