...
def _assist_plan(text: str) -> dict:
    ...
    personalization = resolve_assist_profile(task_kind=task_kind)
    ...
    context = {
        ...
        'planning_caution': personalization['approval_profile']['recommended_caution'],
    }
    success_criteria = [
...
def _unsupported_plan(text: str) -> dict:
    supported = [
        'Summarize this',
        ...
        'Run python script at "/path/to/script.py"',
        'Search <query> and give me key points',
        'Open Gmail',
        'Find flights from SFO to JFK on 2026-07-01 return 2026-07-10',
    ]
    return {
        'goal': 'Unsupported command',
        'signature': 'unsupported:command',
        'context': {
            'unsupported_command': text,
            'unsupported_reason': 'AURA private alpha only supports the current assist family, the existing code-run path, and a small set of web utility flows.',
        ...
        'clarifications': [],
        'memory_hints': [],
        'learning_hints': {'workflow': [], 'preferences': [], 'site': [], 'safety': []},
        'slots': {},
    }
