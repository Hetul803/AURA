...
def execute_steps(run_id: str, steps, event_cb, wait_poll_ms: int = 100):
    out = []
    for step in steps:
        if is_run_cancelled(run_id):
            event_cb({'type': 'run_cancelled', 'run_id': run_id, 'status': 'cancelled'})
            return out + [{'step': step.id, 'status': 'cancelled'}]
        ...
        out.append({'step': step.id, 'status': status, 'result': res})
