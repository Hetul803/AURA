...
from .timing import set_hero_phase
...
def _update_assist_context(run_id: str, step, result: dict):
    ...
    if step.action_type == ASSIST_CAPTURE:
        ...
        assist_state.update({
            ...
            'personalization_profile': draft.get('personalization_profile', {}),
            'learning_signals': draft.get('learning_signals_applied', {}),
        })
        payload['assist'] = assist_state
    elif step.action_type == ASSIST_PASTE:
        observation = result.get('observation') or {}
        copied = bool(observation.get('demo_copy_fallback'))
        paste_state = {
            'status': 'copied' if copied else ('pasted' if result.get('ok') else 'failed'),
            'pasted_length': result.get('result', {}).get('pasted', result.get('pasted', 0)),
            'copied_length': result.get('result', {}).get('copied', 0),
        ...
        approval_state['status'] = 'copied' if copied else ('pasted' if result.get('ok') else approval_state.get('status', 'approved'))
        payload['approval_state'] = approval_state
        assist_state['paste_validation'] = paste_state
        assist_state['final_outcome'] = 'copied' if copied else ('pasted' if result.get('ok') else 'paste_blocked')
        payload['assist'] = assist_state
    if payload:
...
def execute_steps(run_id: str, steps, event_cb, wait_poll_ms: int = 100, start_index: int = 0):
    ...
    for idx, step in enumerate(steps[start_index:], start=start_index):
        ...
        if step.action_type == ASSIST_CAPTURE:
            set_hero_phase(run_id, 'capturing', label='Capturing context', detail='Gathering the active app context for this request.')
        elif step.action_type in (ASSIST_RESEARCH, ASSIST_DRAFT):
            set_hero_phase(run_id, 'drafting', label='Drafting', detail='Preparing the best response for the captured context.')
        elif step.action_type == ASSIST_PASTE:
            set_hero_phase(run_id, 'pasting', label='Pasting', detail='Restoring the original target and applying the approved draft.')
        ...
        while attempts < max_attempts:
            ...
            for guardian_event in guardian_events:
                event_cb({**guardian_event, 'guardian_event_type': guardian_event.get('type'), 'type': 'guardian_event'})
