...
SEVERITY_SCORE = {"info": 5, "low": 20, "medium": 45, "high": 75, "critical": 95, "blocked": 100}
    ...
    for hunk in file.hunks:
        for line in hunk.lines:
            ...
            if detect_secret(content):
                findings.append(RiskFinding("secret_in_diff", "blocked", file.path, "This added line looks like a secret, token, password, private key, or credential.", line.new_lineno, content))
            ...
            if re.search(r"(?i)\b(npm|pnpm|yarn|pip|curl|wget|bash|sh|sudo|rm|dd|mkfs)\b", content):
                ...
                if shell.get("blocked") or shell.get("risk") in {"high", "blocked"}:
                    findings.append(RiskFinding("destructive_shell_command", "blocked" if shell.get("blocked") else "high", file.path, f"Added shell-like text is risky: {shell.get('reason')}.", line.new_lineno, content))
    return findings
...
def _file_findings(file: ChangedFile) -> list[RiskFinding]:
    ...
    if _is_path(path, ".github/workflows", "dockerfile", "docker-compose", "vercel", "netlify", "fly.toml", "render.yaml"):
        findings.append(RiskFinding("deploy_config_change", "medium", path, "Deployment, CI, or hosting configuration changed."))
    ...
    if _is_path(path, "migration", "migrations", "alembic") and not re.search(r"(?i)(rollback|downgrade|down)", file.added_text):
        findings.append(RiskFinding("db_migration_without_migration_file", "medium", path, "Database migration changed without an obvious rollback/downgrade path."))
    return findings
...
def analyze_diff(diff: str | ParsedDiff, *, constitution: Constitution | None = None) -> DiffRiskReport:
    ...
    if len(parsed.files) >= 12 or parsed.additions + parsed.deletions >= 800:
        findings.append(RiskFinding("large_multifile_change", "medium", "*", "This is a large multi-file change; require careful review."))
